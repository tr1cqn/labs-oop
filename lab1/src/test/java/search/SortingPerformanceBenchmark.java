...
/**
 * Бенчмарк для сравнения скорости выполнения сортировки
 */
public class SortingPerformanceBenchmark {
    private static DataSearchService searchService;
    private static List<User> largeUserList;
    private static List<Function> largeFunctionList;
    private static List<Point> largePointList;
    ...
    static void setUp() {
        ...
        for (int i = 0; i < DATA_SIZE; i++) {
            largeUserList.add(new User("user" + random.nextInt(100000), 
        ...
        for (int i = 0; i < DATA_SIZE; i++) {
            largeFunctionList.add(new Function(testUser, "function" + random.nextInt(100000), 
                "type" + (i % 10)));
        }
        
        Function testFunction = new Function(testUser, "test", "test");
        largePointList = new ArrayList<>();
        for (int i = 0; i < DATA_SIZE; i++) {
            largePointList.add(new Point(testFunction, random.nextDouble() * 1000, 
    ...
    void benchmarkSortUsersByLogin() {
        ...
        List<User> sorted = searchService.sortResults(new ArrayList<>(largeUserList), criteria);
    ...
    void benchmarkSortFunctionsByName() {
        ...
        long startTime = System.currentTimeMillis();
        List<Function> sorted = searchService.sortResults(new ArrayList<>(largeFunctionList), criteria);
        long endTime = System.currentTimeMillis();
    ...
    void benchmarkSortPointsByXValue() {
        ...
        long startTime = System.currentTimeMillis();
        List<Point> sorted = searchService.sortResults(new ArrayList<>(largePointList), criteria);
        long endTime = System.currentTimeMillis();
    ...
    void benchmarkAscVsDesc() {
        ...
        List<Point> sortedAsc = searchService.sortResults(new ArrayList<>(largePointList), criteriaAsc);
        long endAsc = System.currentTimeMillis();
        
        long startDesc = System.currentTimeMillis();
        List<Point> sortedDesc = searchService.sortResults(new ArrayList<>(largePointList), criteriaDesc);
